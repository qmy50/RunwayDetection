...
class EMA(nn.Module):
    def __init__(self, channels, c2=None, factor=32):
        ...
        self.softmax = nn.Softmax(-1)   #dim=-1        
        self.agp = nn.AdaptiveAvgPool2d((1, 1))
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))       # 沿着宽度方向压缩，保留高度信息（输出形状为H*1) 
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))       # 沿着高度方向压缩，输出1*W
    ...
    def forward(self, x):
        ...
        hw = self.conv1x1(torch.cat([x_h, x_w], dim=2)) # b×g,c/g,h+w,1 -> b×g,c/g,h+w,1
        x_h, x_w = torch.split(hw, [h, w], dim=2)       # 形状恢复为 b*g,c//g,h,1  b*g,c//g,w,1
