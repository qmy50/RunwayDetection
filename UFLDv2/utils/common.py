...
def get_args():
    ...
    parser.add_argument('--finetune', default = 'model/tusimple_res18.pth'    , type = str)
...
def merge_config():
    ...
    if cfg.dataset == 'CULane':
        ...
        cfg.row_anchor = np.linspace(260,1080, cfg.num_row)/1080
        cfg.col_anchor = np.linspace(0,1, cfg.num_col)
...
# def get_train_loader(cfg, distributed=False):
#     if cfg.dataset == 'CULane':
#         train_loader, cls_num = _get_train_loader(
#             cfg.batch_size, cfg.data_root, cfg.griding_num, cfg.dataset, cfg.use_aux, distributed, cfg.num_lanes,
#             cfg.train_width, cfg.train_height, cfg.num_cell_row, cfg.num_cell_col, cfg.crop_ratio)
#     elif cfg.dataset == 'Tusimple':
#         train_loader, cls_num = _get_train_loader(
#             cfg.batch_size, cfg.data_root, cfg.griding_num, cfg.dataset, cfg.use_aux, distributed, cfg.num_lanes,
#             cfg.train_width, cfg.train_height, cfg.num_cell_row, cfg.num_cell_col, cfg.crop_ratio)
...
#     return train_loader, cls_num

def get_train_loader(cfg):
    if cfg.dataset == 'Tusimple':
        distributed = False
        #get_train_loader_v1(batch_size, data_root, distributed=False, num_lanes=4):
        train_loader = get_train_loader_v1(cfg.batch_size, cfg.data_root, distributed,cfg.num_lanes)
        return train_loader
    raise NotImplementedError
...
def inference_culane_tusimple(net, data_label):
    # ========== 第一步：先将输入图像迁移到GPU（核心修正） ==========
    imgs = data_label['images'].cuda()  # 图像是输入，必须先迁GPU
    pred = net(imgs)  # 模型输出自动在GPU，无需再.cuda()

    # ========== 第二步：生成扩展标签（先在CPU处理，再迁GPU） ==========
    cls_out_ext_label = (data_label['labels_row'] != -1).long()
    cls_out_col_ext_label = (data_label['labels_col'] != -1).long()

    # ========== 第三步：构建结果字典（仅CPU tensor迁GPU，GPU tensor直接用） ==========
    res_dict = {
        # 行方向预测/标签：pred已在GPU，data_label的tensor迁GPU
        'cls_out': pred['loc_row'],
        'cls_label': data_label['labels_row'].cuda(),
        # 列方向预测/标签
        'cls_out_col': pred['loc_col'],
        'cls_label_col': data_label['labels_col'].cuda(),
        # 行/列存在性预测/标签
        'cls_out_ext': pred['exist_row'],
        'cls_out_ext_label': cls_out_ext_label.cuda(),
        'cls_out_col_ext': pred['exist_col'],
        'cls_out_col_ext_label': cls_out_col_ext_label.cuda(),
        # 分割分支（核心：统一key为seg_images，和dataset对齐）
        'labels_row_float':data_label['labels_row_float'].cuda(), 'labels_col_float':data_label['labels_col_float'].cuda()
    }

    # ========== 修正key名称：seg_out 分支（dataset里是seg_images，不是segs） ==========
    if 'seg_out' in pred.keys():
        res_dict['seg_out'] = pred['seg_out']  # pred已在GPU，无需.cuda()
        # 注意：dataset里分割图的key是seg_images，不是segs！
        res_dict['seg_label'] = data_label['seg_images'].cuda()  
...
def inference_curvelanes(net, data_label):
    ...
    res_dict = {'cls_out': pred['loc_row'], 'cls_label': data_label['labels_row'].cuda(), 'cls_out_col':pred['loc_col'],'cls_label_col':data_label['labels_col'].cuda(),
                'cls_out_ext':pred['exist_row'], 'cls_out_ext_label':cls_out_ext_label.cuda(), 'cls_out_col_ext':pred['exist_col'],
                'cls_out_col_ext_label':cls_out_col_ext_label.cuda(), 'seg_label': data_label['seg_images'].cuda(), 'seg_out_row': pred['lane_token_row'], 'seg_out_col': pred['lane_token_col'] }
    if 'seg_out' in pred.keys():
        res_dict['seg_out'] = pred['seg_out']
        res_dict['seg_label'] = data_label['segs'].cuda()
    return res_dict

def calc_loss(loss_dict, results, logger, global_step, epoch):
    loss = 0
    if 'seg_out' in results and 'seg_label' in results:
        print("=== 分割任务维度验证 ===")
        print(f"seg_out形状: {results['seg_out'].shape}")  # 看第二个维度（C）
