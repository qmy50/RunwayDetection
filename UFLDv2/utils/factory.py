from UFLDv2.utils.loss import SoftmaxFocalLoss, ParsingRelationLoss, ParsingRelationDis, MeanLoss, TokenSegLoss, VarLoss, EMDLoss, RegLoss
from UFLDv2.utils.metrics import MultiLabelAcc, AccTopk, Metric_mIoU, Mae
from UFLDv2.utils.dist_utils import DistSummaryWriter
...
def get_loss_dict(cfg):
    if cfg.dataset == 'CurveLanes':
        ...
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'relation_dis','cls_loss _col','cls_ext','cls_ext_col', 'mean_loss_row', 'mean_loss_col'],
        ...
        # loss_dict = {
        #     'name': ['relation_loss', 'relation_dis','cls_ext','cls_ext_col', 'mean_loss_row', 'mean_loss_col'],
        #     'op': [ ParsingRelationLoss(), ParsingRelationDis(),  torch.nn.CrossEntropyLoss(),  torch.nn.CrossEntropyLoss(), MeanLoss(), MeanLoss(),],
        #     'weight': [ cfg.sim_loss_w, cfg.shp_loss_w,  1.0, 1.0, cfg.mean_loss_w, cfg.mean_loss_w,],
    ...
    if cfg.use_aux:
        ...
        loss_dict['op'].append(torch.nn.CrossEntropyLoss(weight = torch.tensor([0.6, 1., 1.])).cuda())
