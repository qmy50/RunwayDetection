...
class AccTopk():
    ...
    def update(self, predict, target):
        predict = predict.argmax(1)
        predict, target = converter(predict), converter(target)
        valid_mask = (target != self.background_classes)
        valid_target = target[valid_mask]
        valid_predict = predict[valid_mask]
        self.cnt += len(valid_target)
        self.top5_correct += np.sum(np.abs(valid_predict - valid_target) < self.k)
    def get(self):
        return self.top5_correct / self.cnt if self.cnt > 0 else 0.0
